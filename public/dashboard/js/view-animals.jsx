...
(function(){
  ...
  function AnimalsView(props) {
    ...
    return h('div', { className: 'dash-page' },
      ...
      loading ? h(LoadingBlock, null, 'Loading animals...') : filtered.length === 0 ? h(EmptyPanel, { iconName:'search', message:'No animals match this search or filter.' }) : viewMode === 'grid' ? h('div', { style:{ display:'grid', gridTemplateColumns:columns, gap:isMobile ? 14 : 16 } }, filtered.map(function(a){ return h(AnimalCard, { key:a.id, animal:a, isMobile:isMobile, onOpen:function(row){ if (onNavigate) onNavigate('animal-profile', { animalId:row.id }); }, onDelete:deleteAnimal }); })) : h(SoftCard, { style:{ overflowX:'auto' } }, renderAnimalTable(filtered, onNavigate, deleteAnimal)),
  ...
  function EditDrawer(props) {
    ...
    var panel = h('div', {
      style: {
        ...
        maxHeight: '100vh',
        background: u.bg,
        borderLeft: isMobile ? 'none' : '1px solid ' + u.border,
        boxSizing: 'border-box',
        display: 'flex',
        flexDirection: 'column',
        overflow: 'hidden',
      ...
      h('div', {
        ...
        h('h2', { style:{ margin:0, color:u.text, fontSize:18 } }, 'Edit Profile'),
        h('div', { style:{ display:'flex', alignItems:'center', gap:8 } },
          h(Button, { variant:'secondary', size:'sm', onClick:onClose }, 'Cancel'),
          h(Button, { size:'sm', disabled:saving, onClick:save }, saving ? 'Saving...' : 'Save')
        )
      ),
      h('div', {
        style: {
